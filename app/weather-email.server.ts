...
export async function sendHeatAdvisoryEmail(
  email: string,
  firstName: string,
  orderName: string,
  logoUrl?: string,
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0; padding:0; background-color:#f7f3ee;">
  <tr>
    ...
    await makeTransporter().sendMail({
      from: process.env.OUTLOOK_EMAIL,
      to: email,
      subject: `Heat Advisory: Shipping Delays for Order ${orderName}`,
      html,
    });

    console.log(`[Email] Heat advisory email sent to ${email} for order ${orderName}`);
    return true;
  } catch (error) {
    console.error(`[Email] Error sending heat advisory email to ${email}:`, error);
    return false;
  }
}
