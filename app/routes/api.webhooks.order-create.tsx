  ...
  try {
    ...
    if (slip.alert && slip.alert.level === "danger") {
      ...
      if (tagResult.data?.tagsAdd?.userErrors?.length) {
        ...
        console.log(`[Webhook] Successfully tagged order ${orderName}`);

        // Send weather delay email to customer using branded template
        if (slip.order.customerEmail) {
          try {
            const firstName = slip.order.customerName?.split(" ")[0] || "there";
            const shopName = "Superior Shrimp & Aquatics";

            await transporter.sendMail({
              from: process.env.OUTLOOK_EMAIL,
              to: slip.order.customerEmail,
              subject: `Your order #${orderName} — Weather delay`,
              html: `<!-- Superior Shrimp & Aquatics - Weather Delay Email -->
      ...
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px; max-width:600px; background-color:#ffffff; border-radius:14px; overflow:hidden; border:1px solid #e8e2da;">
        <tr>
          <td style="background-color:#b50707; background-image:linear-gradient(160deg,#b50707 0%,#F40909 100%); padding:32px 36px; text-align:center;">
            <div style="font-family:'Playfair Display',Georgia,serif; font-size:26px; font-weight:700; color:#ffffff; line-height:1.2;">${shopName}</div>
        ...
        <tr>
          <td style="background-color:#111111; padding:28px 36px; text-align:center;">
            <p style="font-family:'Playfair Display',Georgia,serif; font-size:16px; font-weight:600; color:#ffffff; margin:0 0 6px 0;">${shopName}</p>
            ...
            console.log(`[Webhook] Weather delay email sent to ${slip.order.customerEmail}`);
          } catch (emailError) {
